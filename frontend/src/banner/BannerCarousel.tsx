                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center h-full px-6 lg:px-20 py-8 gap-8">
                        ...
                        <div className="text-left max-w-lg">
                            <h2 className="text-5xl sm:text-6xl font-extrabold tracking-tight leading-tight drop-shadow-xl">
                                <span className="text-white">KÖP FLER,</span> <br />
                                <span className="text-[#7CD038]">SPARA MER!</span>
                            </h2>
                        ...
                        <div className="space-y-6 text-right max-w-xs hidden lg:block">
                            {banner.offers.map((offer, i) => (
                                <div key={i} className="bg-[#002a66] rounded-full px-6 py-4 shadow-lg">
                                    <p className="text-3xl font-bold text-[#7CD038] leading-tight">{offer.title}</p>
                                    <p className="text-white text-lg">{offer.detail}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {banners.map((_, idx) => (
                    <button
                        ...
                        className={`w-3 h-3 rounded-full transition duration-300 ${
                            idx === currentIndex ? "bg-white scale-125" : "bg-gray-400 opacity-60 hover:opacity-100"
                        }`}
